...
utils::NoiseMap WorldGenerator::generateChunk(double xMin, double xMax, double yMin, double yMax, double sizeMod)
{
	using namespace noise;
	std::string saveFolderName = "regionMaps";
	_mkdir(saveFolderName.c_str());

	std::stringstream ssFilename;
	ssFilename << saveFolderName << "/"
		<< (int)xMin << "_" << (int)yMin << "_"
		<< (int)xMax << "_" << (int)yMax << "/";
	...
	utils::NoiseMapBuilderPlane planet;
	utils::NoiseMap elevGrid;

	// Pass in the boundaries of the elevation grid to extract.
	planet.SetBounds(xMin/sizeMod, xMax/sizeMod, yMin/sizeMod, yMax/sizeMod);
	planet.SetDestSize(xMax-xMin+1, yMax-yMin+1);
	...
	double degExtent = 180;
	double gridExtent = (double)256;
	...
	bitmapWriter.SetDestFilename(filename+"terrain.bmp");
	...
	// Write the image as a Windows bitmap file (*.bmp).
	bitmapWriter.SetSourceImage(destImage);
	bitmapWriter.SetDestFilename(filename+"terrainsurface.bmp");
	bitmapWriter.WriteDestFile();
	...
	bitmapWriter.SetSourceImage(destImage);
	bitmapWriter.SetDestFilename(filename+"terrainspec.bmp");
	bitmapWriter.WriteDestFile();
	...
	bitmapWriter.SetSourceImage(destImage);
	bitmapWriter.SetDestFilename(filename+"terrainnormal.bmp");
	bitmapWriter.WriteDestFile();
